...
std::unique_ptr<OpponentProjectile> Opponent::LaunchProjectile() {
  std::unique_ptr<OpponentProjectile> unique_opponent_projectile =
     std::make_unique<OpponentProjectile>(Opponent::GetX(), Opponent::GetY());
  if(counter == 10) {
  counter = 0;
  return unique_opponent_projectile;
  } else {
  counter ++;
  return nullptr;
  }
...
OpponentProjectile::OpponentProjectile(int x, int y) : GameElement(x, y, 5, 5) {}
